...
import subprocess
from knowledge_handler.knowledge_update_localLM import KGUpdate
...
from knowledge_handler.knowledge_preparation_localLM import KGPre
from knowledge_handler.knowledge_transformation_localLM import KGTrans
from space_optimizer.knob_selection_localLM import KnobSelection
...
if __name__ == '__main__':
    ...
    # write your huggingface access token
    ...
    if args.process == 'whole' or args.process == 'knowledge':
        ...
        knob_selection = KnobSelection(db=args.db, dbms=dbms, benchmark=args.test, knowledge_path=folder_path, access_token=access_token, model=args.model)
        ...
        knowledge_pre = KGPre(db=args.db, access_token=access_token, model=args.model, knowledge_path=os.path.join(f"{folder_path}", f"knowledge_collection/{args.db}"))
        knowledge_trans = KGTrans(db=args.db, access_token=access_token, model=args.model, knowledge_path=os.path.join(f"{folder_path}", f"knowledge_collection/{args.db}"))
        knowledge_update = KGUpdate(db=args.db, access_token=access_token, model=args.model, knowledge_path=os.path.join(f"{folder_path}", f"knowledge_collection/{args.db}"))
