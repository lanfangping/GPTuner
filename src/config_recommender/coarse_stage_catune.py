...
class CoarseStageCATune(CoarseSpace):
    ...
    def __init__(self, dbms, test, timeout, target_knobs_path, special_skill_path, results_folder, seed, log, rules=[], conditional_activations=[]):
        ...
        knob_info_path = os.path.join(os.path.dirname(os.path.dirname(special_skill_path)), 'knob_info/official_document.json')
        self.all_knob_info = json.load(open(knob_info_path, "r"))
        ...
        for knob in self.target_knobs:
            ...
            if file_name in os.listdir(self.skill_path):
                ...
                if knob_type == "integer":  
                    ...
                    self.log.info(f"Coarse search space for knob {knob}: {sequence}")
                    sequence = [unify_unit(value, info['unit']) for value in sequence]
                    self.log.info(f"Unified coarse search space for knob {knob}: {sequence}")
                    sequence = list(set(sequence))
                    sequence.sort()
                    unified_default_value = unify_unit(boot_value, info['unit'])
                    normal_para = CategoricalHyperparameter(
                        knob,
                        [value for value in sequence],
                        default_value = unified_default_value,
                    )
